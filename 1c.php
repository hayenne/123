...
if($verso->request->get('type') == 'sale' && $verso->request->get('mode') == 'init')
{
	$tmp_files = glob($dir.'*.*');
...
if($verso->request->get('type') == 'sale' && $verso->request->get('mode') == 'file')
{
	$filename = $verso->request->get('filename');
	...
	foreach($xml->Документ as $xml_order)
	{
		...
		$existed_order = $verso->orders->get_order(intval($order->id));
		...
		if($existed_order)
		{
			$verso->orders->update_order($order->id, $order);
		}
		else
		{
			$order->id = $verso->orders->add_order($order);
		}
		...
		foreach($xml_order->Товары->Товар as $xml_product)
		{
			...
			// Ищем товар
			$verso->db->query('SELECT id FROM __products WHERE external_id=?', $product_1c_id);
			$product_id = $verso->db->result('id');
			$verso->db->query('SELECT id FROM __variants WHERE external_id=? AND product_id=?', $variant_1c_id, $product_id);
			$variant_id = $verso->db->result('id');
			...
			$verso->db->query('SELECT id FROM __purchases WHERE order_id=? AND product_id=? AND variant_id=?', $order->id, $product_id, $variant_id);
			$purchase_id = $verso->db->result('id');
			if(!empty($purchase_id))
				$purchase_id = $verso->orders->update_purchase($purchase_id, $purchase);
			else
				$purchase_id = $verso->orders->add_purchase($purchase);
			$purchases_ids[] = $purchase_id;
		}
		// Удалим покупки, которых нет в файле
		foreach($verso->orders->get_purchases(array('order_id'=>intval($order->id))) as $purchase)
		{
			if(!in_array($purchase->id, $purchases_ids))
				$verso->orders->delete_purchase($purchase->id);
		}
		
		$verso->db->query('UPDATE __orders SET discount=0, total_price=? WHERE id=? LIMIT 1', $xml_order->Сумма, $order->id);
		
	}
	

	print "success";
	$verso->settings->last_1c_orders_export_date = date("Y-m-d H:i:s");

}

if($verso->request->get('type') == 'sale' && $verso->request->get('mode') == 'query')
{
		...
		$orders = $verso->orders->get_orders(array('modified_from'=>$verso->settings->last_1c_orders_export_date));
		foreach($orders as $order)
		{
			...
			$purchases = $verso->orders->get_purchases(array('order_id'=>intval($order->id)));
			...
			foreach($purchases as $purchase)
			{
				...
				{
					$verso->db->query('SELECT external_id FROM __products WHERE id=?', $purchase->product_id);
					$id_p = $verso->db->result('external_id');
					$verso->db->query('SELECT external_id FROM __variants WHERE id=?', $purchase->variant_id);
					$id_v = $verso->db->result('external_id');
					...
					{
						$verso->db->query('UPDATE __products SET external_id=id WHERE id=?', $purchase->product_id);
					...
					{
						$verso->db->query('UPDATE __variants SET external_id=id WHERE id=?', $purchase->variant_id);
...
if($verso->request->get('type') == 'sale' && $verso->request->get('mode') == 'success')
{
		$verso->settings->last_1c_orders_export_date = date("Y-m-d H:i:s");
}


if($verso->request->get('type') == 'catalog' && $verso->request->get('mode') == 'checkauth')
{
	print "success\n";
	print session_name()."\n";
	print session_id();
...
if($verso->request->get('type') == 'catalog' && $verso->request->get('mode') == 'init')
{	
	$tmp_files = glob($dir.'*.*');
	if(is_array($tmp_files))
	foreach($tmp_files as $v)
...
if($verso->request->get('type') == 'catalog' && $verso->request->get('mode') == 'file')
{
	$filename = basename($verso->request->get('filename'));
	$f = fopen($dir.$filename, 'ab');
	fwrite($f, file_get_contents('php://input'));
	fclose($f);
	print "success\n";
...
if($verso->request->get('type') == 'catalog' && $verso->request->get('mode') == 'import')
{
	$filename = basename($verso->request->get('filename'));
...
function import_categories($xml, $parent_id = 0)
{
	...
	{
		$verso->db->query('SELECT id FROM __categories WHERE external_id=?', $xml_group->Ид);
		$category_id = $verso->db->result('id');
		if(empty($category_id))
			$category_id = $verso->categories->add_category(array('parent_id'=>$parent_id, 'external_id'=>$xml_group->Ид, 'url'=>translit($xml_group->Наименование), 'name'=>$xml_group->Наименование, 'meta_title'=>$xml_group->Наименование, 'meta_keywords'=>$xml_group->Наименование, 'meta_description'=>$xml_group->Наименование ));
...
function import_features($xml)
{
	...
	foreach ($property as $xml_feature)
	{
		...
		{
			$verso->db->query('SELECT id FROM __features WHERE name=?', strval($xml_feature->Наименование));
			$feature_id = $verso->db->result('id');
			if(empty($feature_id))
				$feature_id = $verso->features->add_feature(array('name'=>strval($xml_feature->Наименование)));
...
function import_product($xml_product)
{
	...
	{
		$verso->db->query('SELECT product_id, id FROM __variants WHERE sku=?', $variant->sku);
		$res = $verso->db->result();
	...
	if(empty($product_id))
	{
		...
		$verso->categories->add_product_category($product_id, $category_id);
		...
		if(isset($xml_product->Картинка))
		{
			foreach($xml_product->Картинка as $img)
			{
				...
				if(!empty($image) && is_file($dir.$image) && is_writable($verso->config->original_images_dir))
				{
					rename($dir.$image, $verso->config->original_images_dir.$image);
					$verso->products->add_image($product_id, $image);
				}
			}
		}
	...
	else
	{
		...
		{
			$verso->db->query('SELECT id FROM __variants WHERE product_id=?', $product_id);
			$variant_id = $verso->db->result('id');		
		}
		...
		if($full_update)
		{
			...
			{
   	    		$query = $verso->db->placehold('DELETE FROM __products_categories WHERE product_id=?', $product_id);
   	    		$verso->db->query($query);
				$verso->categories->add_product_category($product_id, $category_id);
			}
		...
		if(isset($xml_product->Картинка))
		{
			foreach($xml_product->Картинка as $img)
			{
				...
				if(!empty($image) && is_file($dir.$image) && is_writable($verso->config->original_images_dir))
				{
					$verso->db->query('SELECT id FROM __images WHERE product_id=? ORDER BY position LIMIT 1', $product_id);
					$img_id = $verso->db->result('id');
					if(!empty($img_id))
						$verso->products->delete_image($img_id);
					rename($dir.$image, $verso->config->original_images_dir.$image);
					$verso->products->add_image($product_id, $image);
				}
			}
	...
	if(empty($variant_id))
	{
		...
		$variant->stock = 0;
		$variant_id = $verso->variants->add_variant($variant);
	}
	else
	{
		$verso->variants->update_variant($variant_id, $variant);
	}
	// Свойства товара
	if(isset($xml_product->ЗначенияСвойств->ЗначенияСвойства))
	{
		foreach ($xml_product->ЗначенияСвойств->ЗначенияСвойства as $xml_option)
		{
			if(isset($_SESSION['features_mapping'][strval($xml_option->Ид)]))
			{
				...
				{
					$verso->features->add_feature_category($feature_id, $category_id);
					$values = array();
					foreach($xml_option->Значение as $xml_value)
						$values[] = strval($xml_value);
					$verso->features->update_option($product_id, $feature_id, implode(' ,', $values));
				}
			}
			...
			elseif(isset($_SESSION['brand_option_id']) && !empty($xml_option->Значение))
			{
				...
				// Добавим бренд
				// Найдем его по имени
				$verso->db->query('SELECT id FROM __brands WHERE name=?', $brand_name);
				if(!$brand_id = $verso->db->result('id'))
					// Создадим, если не найден
					$brand_id = $verso->brands->add_brand(array('name'=>$brand_name, 'meta_title'=>$brand_name, 'meta_keywords'=>$brand_name, 'meta_description'=>$brand_name, 'url'=>translit($brand_name)));	
				if(!empty($brand_id))
					$verso->products->update_product($product_id, array('brand_id'=>$brand_id));
			}
		}		
	}
	...
	{
		$verso->variants->delete_variant($variant_id);
		$verso->db->query('SELECT count(id) as variants_num FROM __variants WHERE product_id=?', $product_id);
		if($verso->db->result('variants_num') == 0)
			$verso->products->delete_product($product_id);
...
function import_variant($xml_variant)
{
	...
	$verso->db->query('SELECT v.id FROM __variants v WHERE v.external_id=? AND product_id=(SELECT p.id FROM __products p WHERE p.external_id=? LIMIT 1)', $variant_1c_id, $product_1c_id);
	$variant_id = $verso->db->result('id');
	...
	{
		// Ищем валюту по коду
		$verso->db->query("SELECT id, rate_from, rate_to FROM __currencies WHERE code like ?", $xml_variant->Цены->Цена->Валюта);
		$variant_currency = $verso->db->result();
		// Если не нашли - ищем по обозначению
		if(empty($variant_currency))
		{
			$verso->db->query("SELECT id, rate_from, rate_to FROM __currencies WHERE sign like ?", $xml_variant->Цены->Цена->Валюта);
			$variant_currency = $verso->db->result();
		}
